 ...
 *
 *
 * housewiz_config.c - Access the Wiz configuration.
 *
 * SYNOPSYS:
 *
 ...
 * double      housewiz_config_boolean (int parent, const char *path);
 *
...
static const char *ConfigFile = "/etc/house/wiz.json";

static const char *housewiz_config_refresh (const char *file) {
...
const char *housewiz_config_load (int argc, const char **argv) {
    ...
    return housewiz_config_refresh (ConfigFile);
}
...
int housewiz_config_find (int parent, const char *path, int type) {
    int i;
...
const char *housewiz_config_string (int parent, const char *path) {
    int i = housewiz_config_find(parent, path, PARSER_STRING);
    return (i >= 0) ? ConfigParsed[i].value.string : 0;
}

int housewiz_config_integer (int parent, const char *path) {
    int i = housewiz_config_find(parent, path, PARSER_INTEGER);
    return (i >= 0) ? ConfigParsed[i].value.integer : 0;
}

int housewiz_config_boolean (int parent, const char *path) {
    int i = housewiz_config_find(parent, path, PARSER_BOOL);
    return (i >= 0) ? ConfigParsed[i].value.bool : 0;
}

int housewiz_config_array (int parent, const char *path) {
    return housewiz_config_find(parent, path, PARSER_ARRAY);
}
...
int housewiz_config_object (int parent, const char *path) {
    return housewiz_config_find(parent, path, PARSER_OBJECT);
}
