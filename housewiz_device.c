...
static void housewiz_device_receive (int fd, int mode) {
    ...
    if (size > 0) {
        ...
        if (device < 0) {
            ...
            device = DevicesCount++;
            snprintf (Devices[device].name, sizeof(Devices[0].name),
                      "wiz%d", device+1);
            safecpy (Devices[device].macaddress,
                     mac, sizeof(Devices[0].macaddress));
            safecpy (Devices[device].description,
                     "autogenerated", sizeof(Devices[0].description));
...
void housewiz_device_initialize (int argc, const char **argv, int livestate) {
    LiveState = livestate;
